...
def create_tf_example(inputs, labels):
  ...
  feature_dict = {
      'inputs1': _bytes_feature(inputs1.tobytes()),
      'inputs2': _bytes_feature(inputs2.tobytes()),
...
class DataLoader():
  ...
  def read(self, das_filename, geo_filename):
    with h5py.File(das_filename, 'r') as f:
      das_data = f.get('input')[()]
      labels = f.get('label')[()]
    with h5py.File(geo_filename, 'r') as f:
      geo_data = f.get('input')[()]
    # Experimenting 3
    das_data = np.clip(das_data, -0.024, 0.024) / 0.014088576
    das_data = np.float32(das_data)
    geo_clip = np.array(
  ...
  for i, file_shard in enumerate(file_shards):
    ...
    with tf.io.TFRecordWriter(tfrecord_file, options=options) as writer:
      for filename in file_shard:
        ...
        geo_filename = filename.replace('das', 'geophone')
        geo_filename = geo_filename.replace('_1.h5', '.h5')
        geo_filename = geo_filename.replace('_2.h5', '.h5')
        if os.path.isfile(geo_filename):
          inputs, outputs = data_loader.read(das_filename, geo_filename)
          tf_example = create_tf_example(inputs, outputs)
          writer.write(tf_example.SerializeToString())
        else:
          print('missing geophone data')
