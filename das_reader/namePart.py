...
nCharsPerType = {'year2': 2, 'year4': 4, 'month': 2, 'day': 2, 'hour24start0': 2,
                 'hour24start1': 2, 'minute': 2, 'second': 2, 'millisecond': 3, 'microsecond': 6}
...
class year2(namePart):
  ...
  def __str__(self):
    ...
    return '{:02d}'.format(self.whichYear)
...
class year4(namePart):
  ...
  def __str__(self):
    ...
    return '{:04d}'.format(self.whichYear)
...
class month(namePart):
  ...
  def __init__(self, whichMonth, verbose=False):
    ...
    if(verbose):
      if(whichMonth < 1 or whichMonth > 12):
        print("Warning month shoudl be 1 to 12, but is "+str(whichMonth))
...
class day(namePart):
  def __init__(self, whichDay, whichMonth, whichYear, verbose=False):
    ...
    self.whichMonth = whichMonth
    self.DaysPerMonth = {1: 31, 2: 28, 3: 31, 4: 30, 5: 31,
                         6: 30, 7: 31, 8: 31, 9: 30, 10: 31, 11: 30, 12: 31}
    if(whichYear % 4 == 0 and whichYear % 100 != 0):
      self.DaysPerMonth[2] = 29  # leap years
    if(whichYear % 400 == 0):
      self.DaysPerMonth[2] = 29  # weird exception
    if(verbose):
      if(whichDay < 1 or whichDay > self.DaysPerMonth[self.whichMonth]):
        print("Warning day should be 1 to " +
              str(self.DaysPerMonth[self.whichMonth]) + "for this month, but it is "+str(whichDay))
...
class hour24start0(namePart):
  ...
  def __init__(self, whichHr, verbose=False):
    ...
    if(verbose):
      if(whichHr < 0 or whichHr > 23):
        print("Warning: hour24start0 should be 0 to 23, but is "+str(whichHr))
...
class hour24start1(namePart):
  ...
  def __init__(self, whichHr, verbose=False):
    ...
    if(verbose):
      if(whichHr < 1 or whichHr > 24):
        print("Warning: hour24start1 should be 1 to 24, but is "+str(whichHr))
    self.whichHr = whichHr
...
class minute(namePart):
  ...
  def __init__(self, whichMin, verbose=False):
    ...
    if(verbose):
      if(whichMin < 0 or whichMin > 59):
        print("Warning minute shoudl be 0 to 59, but is "+str(whichMin))
...
class second(namePart):
  ...
  def __init__(self, whichSec, verbose=False):
    ...
    if(verbose):
      if(whichSec < 0 or whichSec > 59):
        print("Warning second shoudl be 0 to 59, but is "+str(whichSec))
...
class millisecond(namePart):
  ...
  def __init__(self, whichMillisec, verbose=False):
    ...
    if(verbose):
      if(whichMillisec < 0 or whichMillisec > 999):
        print("Warning millisecond should be 0 to 999, but is "+str(whichMillisec))
  ...
  def __str__(self):
    ...
    return '{:03d}'.format(int(self.whichMillisec))
...
class microsecond(namePart):
  ...
  def __init__(self, whichMicrosec, verbose=False):
    ...
    if(verbose):
      if(whichMicrosec < 0 or whichMicrosec > 999999):
        print("Warning microsecond should be 0 to 999999, but is "+str(whichMicrosec))
