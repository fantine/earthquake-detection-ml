import copy
import datetime
# note that dictionary nCharsPerType is defined in namePart
...
class fileSet:
  def __init__(self, listOfPartTypes):
    ...
    self.OKPartNames = ['year2', 'year4', 'month', 'day', 'hour24start0',
                        'hour24start1', 'minute', 'second', 'millisecond', 'microsecond']
  ...
  def generateFilename(self, yearVal, monthVal, dayVal, hourVal, minuteVal, secondVal, millisecVal=0, microsecVal=0):
    ...
    getNamePart = {'year2': str(year2(yearVal)), 'year4': str(year4(yearVal)), 'month': str(month(monthVal)), 'day': str(day(dayVal, monthVal, yearVal)), 'hour24start0': str(hour24start0(hourVal)), 'hour24start1': str(
        hour24start1(hourVal)), 'minute': str(minute(minuteVal)), 'second': str(second(secondVal)), 'millisecond': str(millisecond(millisecVal)), 'microsecond': str(microsecond(microsecVal))}
  ...
  def getTimeFromFilename(self, myFilename):
    ...
    myTime = {'year': 2016, 'month': 1, 'day': 1,
              'hour': 0, 'minute': 0, 'second': 0, 'millisecond': 0}
    # how to set the variable that goes into the datetime constructor for each part
    getValueVariable = {'year2': 'year', 'year4': 'year', 'month': 'month', 'day': 'day', 'hour24start0': 'hour',
                        'hour24start1': 'hour', 'minute': 'minute', 'second': 'second', 'millisecond': 'millisecond', 'microsecond': 'microsecond'}
    for part in self.listOfPartTypes:
      if(part in self.OKPartNames):
        myTime[getValueVariable[part]] = self.getValuePart(
            part, currentIndex, myFilename)  # set the time value for this part of the name
        currentIndex = currentIndex + nCharsPerType[part]
      else:  # its a string thats exactly preserved so skip ahead that many characters
        nChars = len(part)
        # check that the filename includes this string
        if(myFilename[currentIndex:currentIndex+nChars] != part):
          print("Warning: characters "+str(currentIndex)+" to "+str(currentIndex+nChars-1) +
                " should be "+part+" but are really "+myFilename[currentIndex:currentIndex+nChars])
